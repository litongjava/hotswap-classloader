 ...
 */
@Slf4j
public class NettyBootRestartServer implements RestartServer {
  ...
  public void restart() {
    ...
    if (TioBootArgument.config != null) {
      context = NettyApplication.run(primarySources, TioBootArgument.config, args);
    } else {
      context = NettyApplication.run(primarySources, args);
    }
